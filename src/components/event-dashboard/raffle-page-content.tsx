...
export function RafflePageContent({ event, activeRaffles, finishedRaffles }: RafflePageContentProps) {
    ...
    const handleDrawWinner = async (raffleId: string) => {
        ...
        const result = await drawRaffleWinner(raffleId, event.id);
        if (result.success) {
            if (result.winner) {
                toast({
                    title: "Winner Drawn!",
        ...
        <Dialog open={openNewRaffleDialog} onOpenChange={setOpenNewRaffleDialog}>
          ...
          <DialogContent className="sm:max-w-[425px]">
            <form onSubmit={handleSubmit(handleCreateRaffle)}>
                ...
                <DialogDescription>
                    Configure a new raffle for the "{event.name}" event. Attendees who have checked-in will be eligible.
                </DialogDescription>
                </DialogHeader>
