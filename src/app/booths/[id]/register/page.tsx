...
import { AttendeeRegistrationForm } from '@/components/booths/attendee-registration-form';
import { Calendar, MapPin, User, Clock } from 'lucide-react';
import { Separator } from '@/components/ui/separator';
import Image from 'next/image';

export default async function BoothRegistrationPage({
  params,
}: {
  params: { id: string };
}) {
  const booth = await getBoothById(params.id);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 lg:gap-12">
          <div className="lg:col-span-2 space-y-6 bg-card p-6 rounded-lg shadow-xl">
            <div className="prose prose-invert max-w-none text-foreground prose-h2:text-foreground prose-h3:text-foreground prose-strong:text-foreground">
              <h2 className="text-2xl font-bold border-b pb-2">About The Booth</h2>
               ...
               <h3 className="text-xl font-bold">Details</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-muted-foreground">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <span>Booth Manager: {booth.booth_manager}</span>
                    </div>
                </div>
            </div>
          </div>
